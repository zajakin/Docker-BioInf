...
RUN sed -i 's/main$/main contrib non-free non-free-firmware/' /etc/apt/sources.list.d/debian.sources && \
  env DEBIAN_FRONTEND=noninteractive apt-get update --allow-releaseinfo-change && \
	...
	apt-get autoclean -y
# GTK 2 and 3 settings for icons and style, wallpaper   # apt-get install -y --no-install-recommends tophat fastx-toolkit cufflinks trnascan-se # lxqt-policykit policykit-1-gnome
