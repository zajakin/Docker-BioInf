...
RUN -nv https://www.rstudio.org/download/latest/stable/server/bionic/rstudio-server-latest-amd64.deb
#RUN curl -o rstudio-server-latest-amd64.deb https://s3.amazonaws.com/rstudio-server/server/bionic/amd64/rstudio-server-2022.07.1-554-amd64.deb
RUN env DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ./rstudio-server-latest-amd64.deb && \
	apt-get autoremove -y && \
	apt-get autoclean -y && \
